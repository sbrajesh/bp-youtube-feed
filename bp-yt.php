...
function bp_yt_screen_group_home() {
    ...
    if( bp_is_group_single() && bp_is_current_action( $bp->yt->slug ) ){
        $bp->yt->is_home = true;
    }
...
function bp_yt_screen_settings_user() {
    ...
    if( bp_is_current_component( $bp->yt->slug ) && bp_is_current_action( 'settings' ) ){
    ...
    //settings screen
        if( ! empty( $_POST['save_settings'] ) ){
            ...
            if( empty( $_POST['yt_account'] ) )
                ...
                update_user_meta(get_current_user_id(), 'yt_account', $_POST['yt_account'] );
...
function bp_yt_group_enable_form() {
    ...
    ?>
    <div class="checkbox">
        <label><input type="checkbox" name="group-enable-yt" id="group-enable-yt" value="1" <?php  checked(1, bp_yt_get_group_prefs() );?>/> <?php _e( 'Enable Youtube Feed', 'bp-yt' ) ?></label>
    </div>
<?php
...
function bp_yt_save_group_prefs( $group_id ) {
      ...
      groups_update_groupmeta( $group_id, 'yt_enabled', (int)$enable );
}
